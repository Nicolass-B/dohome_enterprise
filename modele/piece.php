...
function getCapteursfromPiece(PDO $dbh, $idpiece)
{
    ...
    $query = "SELECT `capteurs`.`ID_Capteurs`, `capteurs`.`Type`, `capteurs`.`Valeur`, `capteurs`.`unite`, `capteurs`.`Etat_Batterie` AS 'batt' 
                  FROM `capteurs` WHERE `capteurs`.`ID_piece`=:idpiece
